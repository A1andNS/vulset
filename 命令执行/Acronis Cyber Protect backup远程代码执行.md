# Acronis Cyber Protect/Backup RCE漏洞

## 漏洞描述
Acronis Cyber​​ Protect 设备在其默认配置中允许在新的端点上匿名注册新的保护/备份代理。此 API 端点还会生成承载令牌，代理随后会使用该令牌向设备进行身份验证。由于管理 Web 控制台与代理的 API 在同一端口上运行，因此此承载令牌对于 Web 控制台上的任何操作也有效。这允许具有设备网络访问权限的攻击者开始注册新代理，检索承载令牌，该令牌提供对 Web 控制台中可用功能的管理员访问权限。Web 控制台包含多种在代理（例如，通过备份的 PreCommands）和设备（例如，通过设备代理上的验证作业）上执行任意命令的可能性。可以使用提供的承载令牌轻松设置这些选项，从而导致所有代理和设备本身完全被攻陷。
> 漏洞编号：CVE-2022-3405
